...
function build_script_generation_prompt(rich_context, instructions, style)
    ...
    ## SCRIPT REQUIREMENTS
    1. $style_instruction
    2. Include comprehensive code mapping using the codelist information provided
    3. Handle all identified transformation requirements (pivoting, range selection, etc.)
    4. Add robust validation using @assert statements for data quality checks
    5. Include detailed comments explaining each transformation step
    6. Handle missing data appropriately
    7. Ensure output strictly conforms to SDMX schema requirements
    8. Include unit tests for key transformations
